...
import type {
  ...
  SuppressionEntry,
  SuppressionMeta,
} from '@/app/(dashboard)/settings/email/shared';
 ...
 */
export function useMailStats(params: { module?: string; startDate?: string; endDate?: string }) {
  const paramsKey = JSON.stringify(params);
  return useApi<LogStats>(() => {
    ...
    if (params.endDate) queryParams.endDate = params.endDate;
    return apiClient
      .get('/mail/logs/stats', { params: queryParams })
      .then((r) => unwrap<LogStats>(r));
  }, [paramsKey]);
}
...
export function useMailSuppressions(page: number) {
  return useApi<SuppressionsResponse>(
    () =>
      apiClient
        .get('/mail/suppressions', { params: { page, limit: 20 } })
