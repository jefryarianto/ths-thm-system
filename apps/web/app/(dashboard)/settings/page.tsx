...
function GoogleOAuthToggle() {
  ...
  const toggleFn = async () => {
    ...
    setEnabled(newVal);
    try {
      await apiClient.post('/settings/auth-providers', { googleOAuthEnabled: newVal });
      toast('success', newVal ? 'Login Google diaktifkan' : 'Login Google dinonaktifkan');
    } catch {
      setEnabled(!newVal);
