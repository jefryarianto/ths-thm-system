...
import { MODULES, EngagementCard } from './shared';
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 animate-pulse h-80"
          />
        ))}
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
          ...
          <p
            className={`text-2xl font-bold ${logsStats.successRate >= 90 ? 'text-green-600' : logsStats.successRate >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
          >
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-6 mt-3 text-xs text-gray-500">
                <span className="flex items-center gap-1.5">
                  <span className="h-3 w-3 rounded-sm bg-green-500" /> Terkirim
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="h-3 w-3 rounded-sm bg-red-500" /> Gagal
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="h-3 w-3 rounded-sm bg-yellow-500" /> Skip
                </span>
              </div>
            </>
          ) : (
        ...
        {engagementLoading ? (
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
              ...
              <EngagementCard
                label="Delivered"
                value={engagement.rates.delivered + '%'}
                color="text-green-600"
              />
              <EngagementCard
                label="Open Rate"
                value={engagement.rates.opened + '%'}
                color="text-indigo-600"
              />
              <EngagementCard
                label="Click Rate"
                value={engagement.rates.clicked + '%'}
                color="text-purple-600"
              />
              <EngagementCard
                label="Bounce Rate"
                value={engagement.rates.bounced + '%'}
                color={engagement.rates.bounced > 5 ? 'text-red-600' : 'text-yellow-600'}
              />
            </div>
            {engagement.dailyTrend &&
              engagement.dailyTrend.some(
                (d: {
                  date: string;
                  sent: number;
                  openRate?: number;
                ...
                <>
                  <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">
                    Tren 7 Hari (Open, Click, Bounce Rate)
                  </p>
                  <ResponsiveContainer width="100%" height={160}>
                    <LineChart
                      data={engagement.dailyTrend}
                  ...
                  <div className="flex items-center justify-center gap-4 mt-1 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                      <span className="h-2.5 w-2.5 rounded-full bg-indigo-500" /> Open
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="h-2.5 w-2.5 rounded-full bg-purple-500" /> Click
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="h-2.5 w-2.5 rounded-full bg-red-500" /> Bounce
                    </span>
                  </div>
                </>
              )}
