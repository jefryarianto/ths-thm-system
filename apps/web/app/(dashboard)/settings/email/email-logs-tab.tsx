    ...
    <div className="space-y-4">
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <p
              className={`text-xl font-bold ${logsStats.successRate >= 90 ? 'text-green-600' : logsStats.successRate >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
            >
        ...
        {logsLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              Riwayat akan muncul setelah email dikirim
            </p>
          </div>
        ) : (
          <>
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div>
                          ...
                          {log.error && (
                            <p
                              className="text-xs text-red-500 dark:text-red-400 mt-0.5 truncate max-w-[200px]"
                              title={log.error}
                            >
                              {log.error}
                            </p>
                      ...
                      <td className="px-5 py-3 hidden sm:table-cell">
                        {(() => {
                          ...
                          const label =
                            MODULES.find((m) => m.value === moduleVal)?.label || String(moduleVal);
                          return (
      ...
      {retryResult && (
        ...
        >
          ...
          <span>
            <strong>Retry selesai:</strong> {retryResult.retried} dicoba, {retryResult.succeeded}{' '}
            berhasil, {retryResult.failed} gagal
          </span>
          <button onClick={() => setRetryResult(null)} className="ml-auto text-xs hover:underline">
            Tutup
          </button>
        </div>
      )}
