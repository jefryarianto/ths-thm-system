...
export default function LetterDetailPanel({
  ...
  useEffect(() => {
    ...
    const endpoint =
      type === 'masuk'
        ? `/letters/incoming/${selectedLetter.id}`
        : `/letters/outgoing/${selectedLetter.id}`;
    apiClient
      .get(endpoint)
      .then(({ data: res }) => {
        if (!cancelled) {
      ...
      {loading ? (
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[detailData.status] || 'bg-gray-100 text-gray-600'}`}
            >
              {detailData.status}
            </span>
          </div>
          ...
          {detailData.isi && (
            <div>
              ...
              <p className="text-sm text-gray-900 dark:text-gray-100 leading-relaxed bg-gray-50 dark:bg-gray-900 rounded-lg p-3 whitespace-pre-wrap">
                {detailData.isi}
              </p>
            </div>
          )}
