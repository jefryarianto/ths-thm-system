    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-4">{error || 'Kegiatan tidak ditemukan'}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{formatTime(activity.tanggalMulai)}</p>
              </div>
            </div>
            <div className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
              ...
              {activity.peserta.map((p) => (
                <div key={p.id} className="flex items-center justify-between px-3 py-2 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{p.anggota.nomorAnggota}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {activity.dokumenKegiatan.map((doc) => (
                  <tr key={doc.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell text-gray-500">{doc.tipe}</td>
                    <td className="px-4 py-3 hidden md:table-cell text-xs text-gray-400">{formatDate(doc.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
