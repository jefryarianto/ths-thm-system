...
test.describe('Dues — /dues', () => {
  ...
  test('renders DataTable with dues rows', async ({ page }) => {
    await expect(page.locator('text=Anggota').first()).toBeVisible();
    await expect(page.locator('text=Periode').first()).toBeVisible();
    await expect(page.locator('text=Jumlah').first()).toBeVisible();
    await expect(page.locator('text=Status').first()).toBeVisible();
    await expect(page.locator('text=Aksi').first()).toBeVisible();
  ...
  test('status badges render for dues statuses', async ({ page }) => {
    ...
    await expect(page.locator('text=Lunas').first()).toBeVisible({ timeout: 8000 });
  });
  ...
  test('charts section renders', async ({ page }) => {
    ...
    await expect(page.locator('text=Feb').first()).toBeVisible();
  });

  test('pagination renders for 50 dues records', async ({ page }) => {
    await page.waitForTimeout(500);
    await expect(page.locator('text=50 total').first()).toBeVisible({ timeout: 8000 });
  });
});
