...
export { escapeHtml }; // re-export for services that import escapeHtml from email-templates
...
export function welcomeMemberEmail(nama: string) {
  return {
    ...
    html: wrap(`
      <h1 style="color: #1a56db;">Selamat Datang, ${escapeHtml(nama)}!</h1>
...
export function approvedMemberEmail(nama: string, nomorAnggota: string) {
  return {
    ...
    html: wrap(`
      <h1 style="color: #1a56db;">Selamat, ${escapeHtml(nama)}!</h1>
      <p>Anda telah resmi menjadi anggota <strong>THS-THM</strong>.</p>
      <p>Nomor Anggota Anda: <strong style="font-size: 18px; color: #1a56db;">${escapeHtml(nomorAnggota)}</strong></p>
...
export function rejectedMemberEmail(
  ...
  return {
    ...
    html: wrap(`
      ...
      ${reason ? `<p>Alasan: <em>${escapeHtml(reason)}</em></p>` : ''}
  ...
  return {
    ...
    html: wrap(`
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="${escapeHtml(resetUrl)}" style="background-color: #1a56db; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
          Reset Password
        </a>
      </div>
...
export function paymentConfirmationEmail(
  ...
  const periodeStr = escapeHtml(periode || '');
...
export function activityInvitationEmail(
  ...
  return {
    subject: `Undangan Kegiatan — ${escapeHtml(activityName)}`,
    html: wrap(`
      <h2 style="color: #1a56db;">📅 Undangan Kegiatan</h2>
      <p>Halo <strong>${escapeHtml(nama)}</strong>,</p>
      <p>Anda telah didaftarkan sebagai peserta kegiatan:</p>
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        <tr><td style="padding: 8px; font-weight: bold; width: 100px;">Kegiatan</td><td style="padding: 8px;">${escapeHtml(activityName)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Tanggal</td><td style="padding: 8px;">${escapeHtml(tanggal)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Lokasi</td><td style="padding: 8px;">${escapeHtml(lokasi)}</td></tr>
      </table>
...
export function trainingNotificationEmail(
  ...
  return {
    subject: `Jadwal Latihan — ${escapeHtml(jenisMateri)}`,
    html: wrap(`
      <h2 style="color: #1a56db;">🏋️ Jadwal Latihan</h2>
      <p>Halo <strong>${escapeHtml(nama)}</strong>,</p>
      <p>Latihan rutin telah dijadwalkan:</p>
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        <tr><td style="padding: 8px; font-weight: bold; width: 100px;">Materi</td><td style="padding: 8px;">${escapeHtml(jenisMateri)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Tanggal</td><td style="padding: 8px;">${escapeHtml(hariTanggal)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Lokasi</td><td style="padding: 8px;">${escapeHtml(lokasi)}</td></tr>
      </table>
...
export function attendanceConfirmationEmail(nama: string, jenisMateri: string, hadir: boolean) {
  return {
    ...
    html: wrap(`
      ...
      <p>Halo <strong>${escapeHtml(nama)}</strong>,</p>
      <p>Kehadiran Anda untuk latihan <strong>${escapeHtml(jenisMateri)}</strong> telah dicatat sebagai <strong>${hadir ? 'HADIR' : 'TIDAK HADIR'}</strong>.</p>
    `),
  };
}
...
export function documentReadyEmail(nama: string, docType: string, nomorDokumen: string) {
  ...
  const label = escapeHtml(docTypeLabels[docType] || docType);
  ...
  return {
    ...
    html: wrap(`
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        ...
        <tr><td style="padding: 8px; font-weight: bold;">Nomor Dokumen</td><td style="padding: 8px;">${escapeHtml(nomorDokumen)}</td></tr>
      </table>
...
export function claimStatusEmail(nama: string, status: string, reason?: string) {
  ...
  const info = statusLabels[status] || {
    ...
    color: '#1a56db',
    title: 'Update Klaim',
    message: `Status klaim Anda: ${escapeHtml(status)}`,
  };
...
export function graduationResultEmail(nama: string, lulus: boolean, skor?: number) {
  return {
    ...
    html: wrap(`
      ...
      ${
        lulus
          ? `<p>Anda telah dinyatakan <strong style="color: #16a34a;">LULUS</strong> dalam ujian pendadaran THS-THM.${skor !== undefined ? ` Total skor: <strong>${escapeHtml(skor)}</strong>` : ''}</p><p>Selamat atas pencapaian ini! Dokumen sertifikat akan segera diproses.</p>`
          : `<p>Hasil ujian pendadaran Anda: <strong style="color: #ca8a04;">BELUM LULUS</strong>.${skor !== undefined ? ` Total skor: <strong>${escapeHtml(skor)}</strong>` : ''}</p><p>Jangan menyerah! Silakan berkoordinasi dengan pelatih untuk persiapan pendadaran ulang.</p>`
      }
    `),
  };
}
...
export function graduationRegisteredEmail(nama: string, namaPendadaran: string, tanggal: string) {
  return {
    ...
    html: wrap(`
      ...
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        <tr><td style="padding: 8px; font-weight: bold; width: 120px;">Pendadaran</td><td style="padding: 8px;">${escapeHtml(namaPendadaran)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Tanggal</td><td style="padding: 8px;">${escapeHtml(tanggal)}</td></tr>
      </table>
...
export function generalNotificationEmail(nama: string, judul: string, isi: string) {
  return {
    subject: `[THS-THM] ${escapeHtml(judul)}`,
    html: wrap(`
      <h2 style="color: #1a56db;">${escapeHtml(judul)}</h2>
      <p>Halo <strong>${escapeHtml(nama)}</strong>,</p>
      <p>${escapeHtml(isi)}</p>
    `),
  };
...
export function examinerWelcomeEmail(nama: string, email: string, setPasswordUrl: string) {
  return {
    ...
    html: wrap(`
      <h2 style="color: #1a56db;">👋 Selamat Datang, ${escapeHtml(nama)}!</h2>
      <p>Akun <strong>Penguji</strong> Anda telah berhasil dibuat di sistem <strong>THS-THM</strong>.</p>
      <p>Email login Anda: <strong>${escapeHtml(email)}</strong></p>
      <p>Klik tombol di bawah untuk membuat password dan mengaktifkan akun Anda:</p>
      <div style="text-align: center; margin: 30px 0;">
        <a href="${escapeHtml(setPasswordUrl)}" style="background-color: #1a56db; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
          Buat Password
        </a>
      </div>
...
export function examinerAssignmentEmail(
  ...
  return {
    subject: `Penugasan Penguji — ${escapeHtml(kegiatanNama)}`,
    html: wrap(`
      <h2 style="color: #1a56db;">📋 Penugasan Penguji</h2>
      <p>Halo <strong>${escapeHtml(nama)}</strong>,</p>
      <p>Anda telah ditugaskan sebagai <strong>${escapeHtml(peran)}</strong> untuk kegiatan:</p>
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        <tr><td style="padding: 8px; font-weight: bold; width: 100px;">Kegiatan</td><td style="padding: 8px;">${escapeHtml(kegiatanNama)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Tanggal</td><td style="padding: 8px;">${escapeHtml(tanggal)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold;">Peran</td><td style="padding: 8px;">${escapeHtml(peran)}</td></tr>
      </table>
...
export function dispositionNotificationEmail(
  ...
  return {
    subject: `Disposisi Surat — ${escapeHtml(perihalSurat)}`,
    html: wrap(`
      <h2 style="color: #1a56db;">📨 Disposisi Surat</h2>
      <p>Halo <strong>${escapeHtml(namaPenerima)}</strong>,</p>
      <p>Anda menerima disposisi surat dari <strong>${escapeHtml(pengirim)}</strong>:</p>
      <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
        <tr><td style="padding: 8px; font-weight: bold; width: 100px;">Perihal</td><td style="padding: 8px;">${escapeHtml(perihalSurat)}</td></tr>
        <tr><td style="padding: 8px; font-weight: bold; vertical-align: top;">Isi</td><td style="padding: 8px;">${escapeHtml(isiDisposisi)}</td></tr>
      </table>
...
export function badgeEarnedEmail(
  ...
  return {
    subject: `${escapeHtml(badgeIcon)} Badge Baru Diraih! — ${escapeHtml(badgeName)}`,
    html: wrap(`
      <h2 style="color: #a855f7;">${escapeHtml(badgeIcon)} Badge Baru!</h2>
      <p>Selamat <strong>${escapeHtml(nama)}</strong>!</p>
      ...
      <div style="text-align: center; margin: 24px 0; padding: 20px; background: #faf5ff; border-radius: 12px;">
        <div style="font-size: 64px; line-height: 1;">${escapeHtml(badgeIcon)}</div>
        <h3 style="color: #7c3aed; margin: 12px 0 4px;">${escapeHtml(badgeName)}</h3>
        <p style="color: #6b7280; margin: 0;">${escapeHtml(description)}</p>
      </div>
...
export function levelUpEmail(nama: string, oldLevel: string, newLevel: string, points: number) {
  ...
  return {
    subject: `🎉 Level Up! ${escapeHtml(nama)} naik ke ${escapeHtml(newLevel)}`,
    html: wrap(`
      ...
      <div style="text-align: center; margin: 24px 0; padding: 20px; background: #fffbeb; border-radius: 12px;">
        <div style="font-size: 32px; margin-bottom: 12px;">
          ${oldEmoji} ${escapeHtml(oldLevel)} → ${newEmoji} ${escapeHtml(newLevel)}
        </div>
        <p style="color: #6b7280; font-size: 18px;">Total Poin: <strong>${escapeHtml(points.toLocaleString('id-ID'))}</strong></p>
      </div>
