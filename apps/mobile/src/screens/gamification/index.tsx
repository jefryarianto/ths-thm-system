...
function PointsChart({ data }: { data: Array<{ month: string; cumulative: number }> }) {
  ...
  const labels = data.filter(
    (_, i) => i % Math.max(1, Math.floor(data.length / 4)) === 0 || i === data.length - 1,
  );
              ...
              <View style={styles.pointsCard}>
                ...
                {profile.level && (
                  <View style={styles.levelBadge}>
                    ...
                    <Text style={[styles.levelName, { color: profile.level.color }]}>
                      {profile.level.name}
                    </Text>
                  </View>
                )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.filterRow}>
            <TouchableOpacity
              ...
              onPress={() => {
                const options = [
                  { label: 'Semua Distrik', value: '' },
                  ...(orgTree || []).map((d) => ({ label: d.nama, value: d.id })),
                ];
            ...
            >
              <Text
                style={[
                  ...
                  selectedDistrik !== '' && styles.filterChipTextActive,
                ]}
                numberOfLines={1}
              >
                {selectedDistrik
                  ? (orgTree || []).find((d) => d.id === selectedDistrik)?.nama || 'Distrik'
                  : '📌 Distrik'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.filterChip,
                selectedWilayah !== '' && styles.filterChipActive,
                selectedDistrik === '' && styles.filterChipDisabled,
              ]}
              onPress={() => {
                const wilayahs =
                ...
                const options = [
                  ...
                  ...wilayahs.map((w) => ({ label: w.nama, value: w.id })),
                ];
            ...
            >
              <Text
                style={[
                  styles.filterChipText,
                  selectedWilayah !== '' && styles.filterChipTextActive,
                ]}
                numberOfLines={1}
              >
                {selectedWilayah
                  ? (orgTree || [])
                      .find((d) => d.id === selectedDistrik)
                      ?.wilayahs?.find((w) => w.id === selectedWilayah)?.nama || 'Wilayah'
                  : '📍 Wilayah'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.filterChip,
                selectedRanting !== '' && styles.filterChipActive,
                selectedWilayah === '' && styles.filterChipDisabled,
              ]}
              onPress={() => {
                const wilayahs =
                  (orgTree || []).find((d) => d.id === selectedDistrik)?.wilayahs || [];
                ...
                const options = [
                  ...
                  ...rantings.map((r) => ({ label: r.nama, value: r.id })),
                ];
            ...
            >
              <Text
                style={[
                  styles.filterChipText,
                  selectedRanting !== '' && styles.filterChipTextActive,
                ]}
                numberOfLines={1}
              >
                {selectedRanting
                  ? (orgTree || [])
                      .find((d) => d.id === selectedDistrik)
                      ?.wilayahs?.find((w) => w.id === selectedWilayah)
                      ?.rantings?.find((r) => r.id === selectedRanting)?.nama || 'Ranting'
                  : '🔴 Ranting'}
              </Text>
            </TouchableOpacity>
